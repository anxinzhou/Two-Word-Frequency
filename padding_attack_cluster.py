...
def pad_wid(wid, to_pad, segments):
        ...
        for index in seg:
            for j in range(wid.shape[1]):
                ...
                if wid[index][j] == 1:
                    padding_set.add(j)
        for index in seg:
            potential_padding_set = padding_set.copy()
            for j in range(wid.shape[1]):
                if wid[index][j] == 1:
                    potential_padding_set.remove(j)
            potential_list = list(potential_padding_set)
            random.shuffle(potential_list)
            amount_to_pad = to_pad[index]
            pad_index = random.sample(potential_list, amount_to_pad)
            for j in pad_index:
                padded_wid[index][j] = 1
    return padded_wid
...
def test_padded_recover(bitmap, padded_bitmap, try_amount):
    ...
    while j < len(padded_bitmap):
        while i + 1 < len(bitmap) and bitmap_count[i + 1][1] <= padded_bitmap_count[j][1]:
            i += 1
        potential_match = []
        padded_count_vector = []
...
for strategy in ["random", "top"]:
    ...
    for minimal_padding in [250, 500, 750, 1000]:
        ...
        length_to_padding, segments = util.to_padding_with_segment(origin_length, minimal_padding)
        # print(length_to_padding)
        print("start prepare padded map")
        padded_wid = pad_wid(wid, length_to_padding, segments)
